...
def count_calls(method: Callable) -> Callable:
    """a decorator script that takes a single method Callable argument
    and returns a Callable"""
    key = method.__qualname__

    @wraps(method)
    def wrapper(self, *args, **kwargs):
        """an increments the count for that key every time the method
...
def replay(fn: Callable):
    """a script that displays the history of calls of a particular function"""
    r = redis.Redis()
...
def call_history(method: Callable) -> Callable:
    """a script that stores the history of inputs and outputs for a particular function
    """
    @wraps(method)
    def wrapper(self, *args, **kwargs):
